...
/*It is used to retrieve whole row from the matrix but it is not used to retrieve the state of  Machine Learning Algorithm*/
    ...
    /*It is used to retrieve whole row from the matrix and it is used to retrieve the state of  Machine Learning Algorithm */
    public static DataSet<Tuple4<Integer, Integer, Double, Double>> getQuery3(ExecutionEnvironment env, String args) throws Exception {
        DataSet<Tuple4<Integer, Integer, Double, Double>> inputDS = env
                .createInput(new CassandraInputFormat<Tuple4<Integer, Integer, Double, Double>>("SELECT row_id, column_id, unique_id, value FROM test.matrix4 where row_id <= "+args+" allow filtering;", new ClusterBuilder() {
                    @Override
                    protected Cluster buildCluster(Cluster.Builder builder) {
                        return builder.addContactPoints("ibm-power-1").build();   //Change ibm-power-1 on cluster otherwise 127.0.0.1
                    }
                }), TupleTypeInfo.of(new TypeHint<Tuple4<Integer, Integer, Double, Double>>() {
                }));
        return inputDS;
